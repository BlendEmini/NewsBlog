...
const OthersRoute = () => {
    ...
    const [othersPosts, setOthersBlog] = useState([]);
    ...
    useEffect(() => {
        async function fetchOthersPosts() {
            try {
                ...
                if (error) {
                    throw new Error(
                        "Error fetching Others blog posts: " + error.message
                    );
                }

                setOthersBlog(data ? data.reverse() : []); // Ensure data is an array, handle null case
            } catch (error) {
